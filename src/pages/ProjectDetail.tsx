    ...
    <div className="min-h-screen bg-bg-body text-theme-text pt-20 pb-24 md:pb-12">
      <div className="max-w-7xl mx-auto px-6">
        <Link to="/work" className="inline-flex items-center gap-2 text-theme-text-muted hover:text-cyan-400 transition-colors mb-8 group">
          ...
          <span className="uppercase tracking-widest text-sm font-bold">{t('work.back_to_work', { defaultValue: 'Back to Work' })}</span>
        </Link>
        ...
        >
          ...
          <div className="p-8 md:p-16 grid grid-cols-1 md:grid-cols-3 gap-12 md:gap-24">
            <div className="md:col-span-2">
              <h3 className="text-sm md:text-base font-bold mb-6 uppercase tracking-wider text-theme-text-dim border-b border-theme-border pb-2">{t('work.overview')}</h3>
              <p className="text-lg md:text-2xl leading-relaxed text-theme-text-muted mb-12 font-light">
                {project.description}
              </p>
              
              <h3 className="text-sm md:text-base font-bold mb-6 uppercase tracking-wider text-theme-text-dim border-b border-theme-border pb-2">{t('work.technologies')}</h3>
            ...
            <div className="space-y-10">
              <div className="grid grid-cols-2 md:grid-cols-1 gap-8">
                <div>
                  <h3 className="text-xs md:text-sm font-bold mb-2 uppercase tracking-wider text-theme-text-dim">{t('work.year')}</h3>
                  <p className="text-2xl md:text-3xl font-mono text-theme-text">{project.year}</p>
                </div>
                <div>
                  <h3 className="text-xs md:text-sm font-bold mb-2 uppercase tracking-wider text-theme-text-dim">{t('work.role')}</h3>
              ...
              <a 
                ...
                className="w-full py-5 bg-theme-text text-bg-body font-bold uppercase tracking-[0.2em] hover:bg-cyan-400 transition-colors block text-center rounded-xl relative overflow-hidden group"
              >
                <span className="relative z-10">{t('work.view_live', { defaultValue: 'View Live Project' })}</span>
        ...
        <div className="mt-24 text-center">
          <h2 className="text-3xl md:text-5xl font-black mb-8 text-theme-text">{t('work.interested', { defaultValue: 'Interested in working together?' })}</h2>
          <Link 
            to="/contact"
            className="inline-block px-12 py-4 border border-theme-border rounded-full text-lg font-bold uppercase tracking-widest hover:bg-theme-text hover:text-bg-body hover:border-theme-text transition-all duration-300"
          >
            {t('work.lets_talk', { defaultValue: "Let's Talk" })}
          </Link>
        </div>
      </div>
    </div>
