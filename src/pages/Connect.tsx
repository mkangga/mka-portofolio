        ...
        {socialLinks.map((link, index) => {
          ...
          return (
            <CardWrapper
              ...
              className={`group relative p-8 ${link.bgColor} border border-theme-border rounded-2xl overflow-hidden transition-all ${
                link.disabled 
            ...
            >
              <div className="relative z-10 flex items-center justify-between">
                <div className="flex items-center gap-6">
                  <div className={`p-4 rounded-xl bg-bg-main/40 border border-theme-border ${link.color} transition-colors`}>
                    <link.icon size={28} />
                  </div>
                  <div>
                    ...
                    <p className="text-theme-text-dim font-mono text-sm uppercase tracking-wider">{link.username}</p>
                  </div>
                </div>
                {!link.disabled && (
                  <ExternalLink size={20} className="text-theme-text-dim group-hover:text-cyan-400 transition-colors" />
                )}
              </div>
